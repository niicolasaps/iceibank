  ...
  return (
    <div style={card}>
      ...
      <form onSubmit={consultar} style={{ display: "flex", gap: 8 }}>
        ...
        <button type="submit" style={btn}>Consultar</button>
      </form>
      <MensagemErro mensagem={erro} onFechar={() => setErro(null)} />
      {conta && <div style={resultado}><b>{conta.nomeAluno}</b> (conta {conta.id}) — Saldo: <b>R$ {conta.saldo.toFixed(2)}</b></div>}
    </div>
  );
}
...
const resultado = { marginTop: 10, padding: 10, background: "#eff6ff", borderRadius: 6, fontSize: 15 };
