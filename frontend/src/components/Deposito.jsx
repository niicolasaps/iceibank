...
export default function Deposito() {
  ...
  return (
    <div style={card}>
      <h3>💰 Depositar</h3>
      <form onSubmit={depositar} style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <button type="submit" style={btn}>Depositar</button>
      </form>
      <MensagemErro mensagem={erro} onFechar={() => setErro(null)} />
      {res && <div style={ok}>✅ Depósito realizado! Novo saldo: <b>R$ {res.saldo.toFixed(2)}</b></div>}
    </div>
  );
}
...
const btn = { padding: "7px 16px", background: "#059669", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 600 };
