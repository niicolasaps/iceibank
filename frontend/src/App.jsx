    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "system-ui, sans-serif" }}>
      ...
      {status && (
        <div style={{ background: "#dbeafe", padding: "8px 24px", fontSize: 13, display: "flex", gap: 24, justifyContent: "center" }}>
          <span>Agência: <b>{status.agencia}</b></span>
          <span>Lamport atual: <b>{status.relogioLamportAtual}</b></span>
          <span>Contas: <b>{status.quantidadeContas}</b></span>
          <button onClick={() => setStatus(null)} style={{ background: "none", border: "none", cursor: "pointer" }}>✕</button>
        </div>
      )}
